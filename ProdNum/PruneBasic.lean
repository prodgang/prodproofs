...
@[simp]
lemma brak_prune_nil_eq_nil (xs : List PreProdNum) : (brak xs) ⊓ nil = nil := by
    cases xs <;> simp only [prune, prune_list_nil_right]


@[simp]
lemma nil_prune_brak_eq_nil (xs : List PreProdNum) : nil ⊓ (brak xs) = nil := by
  cases xs <;> simp only [prune, prune_list_nil_left]
...
lemma prune_list_cons {x y : PreProdNum} {xs ys : List PreProdNum} :
    prune_list (x::xs) (y::ys) = (x ⊓ y) :: prune_list xs ys := by
  simp only [prune_list]
...
lemma prune_eq_zero_iff {x y : PreProdNum } : x ⊓ y = zero ↔ x = zero ∨ y = zero := by
  constructor
  . intro h
    cases x <;> cases y <;> simp only [brak_ne_zero, or_self, or_true, or_false]
    simp only [prune, brak_ne_zero] at h
  . intro h; cases h <;> rename_i h <;> rw [h] <;> simp only [zero_prune, prune_zero]


lemma prune_list_allzero_iff {xs ys : List PreProdNum} :
    allzero (prune_list xs ys) ↔ ∀ i, get xs i = zero ∨ get ys i = zero := by
  constructor
  · intro h i
    have := allzero_get_zero h i
    rwa [get_prune_list, prune_eq_zero_iff] at this
  · intro h
    rw [allzero_iff]
    intro x hx
    obtain ⟨i, hi⟩ := mem_imp_exists_get hx
    rw [← hi, get_prune_list]
    cases h i with
    | inl hgx => rw [hgx, zero_prune]
    | inr hgy => rw [hgy, prune_zero]
...
theorem prune_idem : ∀ x : PreProdNum, x ⊓ x = x := by
  ...
  | h_nil => exact brak_prune_nil_eq_nil _
  | h_cons xs ih =>
...
theorem prune_comm : ∀ x y : PreProdNum, x ⊓ y = y ⊓ x := by
  ...
  case h_nil_left => simp only [nil_prune_brak_eq_nil, brak_prune_nil_eq_nil, implies_true]
  case h_nil_right => simp only [brak_prune_nil_eq_nil, nil_prune_brak_eq_nil, implies_true]
  case h_cons_cons =>
...
theorem prune_assoc : ∀ x y z : PreProdNum, (x ⊓ y) ⊓ z = x ⊓ (y ⊓ z) := by
  ...
  case h_nil_left => simp only [prune, prune_list_nil_left, nil_prune_brak_eq_nil, implies_true];
  case h_nil_mid => simp only [brak_prune_nil_eq_nil, nil_prune_brak_eq_nil, implies_true]
  case h_nil_right => simp only [prune, brak_prune_nil_eq_nil, prune_list_nil_right, implies_true]
...
theorem prune_respects_equiv {x x' y y' : PreProdNum} (hx : x.equiv x') (hy : y.equiv y') : (x ⊓ y).equiv (x' ⊓ y') := by
  ...
  case h_nil1 => intros; simp only [nil_prune_brak_eq_nil]; apply prune_trim_equiv; assumption;
  case h_nil2 => intros; simp only [nil_prune_brak_eq_nil]; apply equiv_symm; apply prune_trim_equiv; apply equiv_symm; assumption;
  case h_nil3 => intros ws xs zs h1 h2; rw [prune_comm (brak xs) (brak zs)]; simp only [brak_prune_nil_eq_nil]; apply prune_trim_equiv; exact h2
  case h_nil4 => intros ws xs zs h1 h2; rw [prune_comm (brak ws) (brak zs)]; simp only [brak_prune_nil_eq_nil]; apply equiv_symm; apply prune_trim_equiv; apply equiv_symm; exact h2
