...
def graft_raw : RawProd → RawProd → RawProd
  | brak xs, brak ys =>
      brak (xs.zipWithAll (fun a b => graft_raw (a.getD zero) (b.getD zero)) ys)
  | zero, y => y
  | x, zero => x
...
theorem graft_raw_idem (x : RawProd) : x ⊔ x = x := by
  ...
  | h_brak xs ih =>
    ...
    | cons y ys ihy =>
      ...
      simp only [graft_raw, List.zipWithAll_cons_cons, Option.getD_some, brak.injEq, List.cons.injEq]
      constructor
      . exact left
...
theorem graft_raw_assoc : ∀ x y z, x ⊔ (y ⊔ z) = (x ⊔ y) ⊔ z := by
  ...
  case h_brak_brak_brak =>
    ...
    | cons x' xx ihxx =>
      ...
      | cons y yy =>
        simp only [graft_raw, List.zipWithAll_cons_cons, Option.getD_some, brak.injEq]
        cases zs with
